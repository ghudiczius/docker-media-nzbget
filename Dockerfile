...
RUN adduser --disabled-password --home=/opt/nzbget --no-create-home --shell /bin/sh --uid 1000 nzbget && \
    ...
    wget --output-document=/tmp/nzbget.run "https://github.com/nzbgetcom/nzbget/releases/download/v${VERSION}/nzbget-${VERSION}-bin-linux.run" && \
...
CMD ["--configfile", "/config/nzbget.conf", "--option", "outputmode=log", "--server"]
